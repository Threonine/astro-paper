...
- Williams' $p+1$
  ...
  - 对于一个一般的模 $p$ 二次非剩余 $A$，如果 $q\neq p$ 我们有 $V_{c(p+1)}\not\equiv 2\pmod{p}$ 但 $V_{c(q+1)}\equiv 2\pmod{q}$（当 $(q-1)/2\mid c$ 时才偶然地不成立）
...
从这个角度看，由 Fermat 小定理可以证明元素 $a^n$ 所在的循环群的元素都是 $\sigma$ 的不动元，也就是素域 $\mathbb{F}_p$ 的 $p$ 个元素， $a^n$ 的 1 次或 2 次系数是 0 似乎是不证自明的了.

## NeSE 升级赛

这题的其中一部分抽象出来是：分解 $n=pq$，但我们不知道 $n$，只知道从同余方程导出的 $kp$ 和 $lq$，其中 $k$ 和 $l$ 都很大，另外有 $p$ 和 $q$ 的关系： $q=4p-1$，且保证 $q$ 也是素数.

类似地我们构造阶为 $q^2-1$ 的群即可. PoC 如下
...
1. 构造一个模 $n$ 的代数群 $\mathbb{R} = \mathbb{Z}_{n}[x]/f(x)$，它在某个 **特别的因子** $q\mid n$ 下「降维」的群 $\mathbb{K} = \mathbb{F}_q[x]/f(x) = \mathbb{F}_{q^2}$ 的 **阶是可预测的**，为 $q^2 - 1 = (q - 1)(q + 1)$；
2. 取群的 **一般的元素 $a$** ，计算「$4n$ 次幂」，得到一个 **特别的元素** $a^{4n}$；
3. 对 $n$ 和这个 **特别的元素** $a^{4n}$，通过 $\text{GCD}(a^n的1次系数,n)$ ，分解出 $q$.

所以我们随机抽一个元素 $a$，得到 $a^{4n}$ 的阶就有可能是 $q^2-1$ 的因子 $q+1$.
